...
export function mailFrom() {
  ...
  if (mailProvider() === 'gmail') return `Fliparo <${GMAIL_USER()}>`;
  if (mailProvider() === 'brevo') return 'Fliparo <verification@fliparo.net>';
  return 'Fliparo <onboarding@resend.dev>';
}

/* Split "Name <a@b.c>" into its parts — Brevo wants them as separate fields. */
function splitAddr(a) {
  const m = String(a).match(/^\s*(.*?)\s*<([^>]+)>\s*$/);
  return m ? { name: m[1] || 'Fliparo', email: m[2].trim() } : { name: 'Fliparo', email: String(a).trim() };
}
...
function buildMessage({ from, to, subject, text, html }) {
  ...
  return [
    ...
    `Message-ID: <${crypto.randomUUID()}@fliparo.net>`,
    'MIME-Version: 1.0',
...
async function sendViaGmail({ to, subject, text, html }) {
  ...
  try {
    ...
    await cmd(sock, 'EHLO fliparo.net', [250], TIMEOUT);
...
async function sendViaBrevo({ to, subject, text, html }) {
  ...
  if (!r.ok) {
    ...
    /* Brevo refuses API calls from IPs it has not seen before. On a PaaS this
       fires the first time you deploy, and again whenever the host moves you
       to a different outbound address — so authorising a single IP is not
       enough. The raw message buries the fix behind a truncated URL. */
    if (r.status === 401 && /IP address/i.test(body)) {
      const ip = (body.match(/(\d{1,3}(?:\.\d{1,3}){3})/) || [])[1];
      throw new Error(
        `Brevo does not recognise this server's IP${ip ? ' (' + ip + ')' : ''}. `
        + 'Authorise it at https://app.brevo.com/security/authorised_ips (note the "s" — the URL in '
        + "Brevo's own error message is misspelled and 404s). Add EVERY outbound IP your host uses "
        + '(Render lists them under Settings → Outbound IP Addresses), or use "Deactivate for API" '
        + 'on that page so a changed IP cannot silently stop your sign-in emails.'
      );
    }
    if (r.status === 401) {
      throw new Error('Brevo rejected the API key. Check BREVO_API_KEY is the v3 API key from '
                    + 'SMTP & API → API Keys, not the SMTP password on that same page.');
    }
    if (r.status === 400 && /sender/i.test(body)) {
      throw new Error('Brevo rejected the sender address. Verify it under Senders, Domains & Dedicated IPs '
                    + 'in the Brevo dashboard, then set MAIL_FROM to that exact address.');
    }
    throw new Error(`Brevo rejected the message (${r.status}): ${body.slice(0, 300)}`);
  }
...
export async function sendCode(to, code) {
  const subject = `${code} is your Fliparo code`;
  const text = `Your Fliparo sign-in code is ${code}\n\n`
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#0b0c0f;padding:32px 16px">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:440px;background:#14161c;border-radius:18px;padding:32px">
        <tr><td style="font:600 13px/1 -apple-system,Segoe UI,sans-serif;letter-spacing:.12em;text-transform:uppercase;color:#d7ff3e;padding-bottom:14px">Fliparo</td></tr>
...
export async function verifyMailLogin() {
  ...
  try {
    ...
    await cmd(sock, 'EHLO fliparo.net', [250], 15_000);
